...
put_test(){
	...
	| mawk -v test_time="$test_time" '/Pub stats: /{printf "p@ %s@ %s@ %s@ %s@ %s@ %s\n", '"$2"', '"$3"', $3, $6, $7, test_time}' \
	| sed -e 's/,//g' -e 's/@/,/g' \
	>> $4
}
...
get_test(){
	...
	| mawk -v test_time="$test_time" '/Sub stats: /{printf "s@ %s@ %s@ %s@ %s@ %s@ %s\n", '"$2"', '"$3"', $3, $6, $7, test_time}' \
	| sed -e 's/,//g' -e 's/@/,/g' \
	>> $4
}
...
put_get_test(){
	...
	| mawk -v test_time="$test_time" '/ Pub stats: /{printf "p@ %s@ %s@ %s@ %s@ %s@ %s\n", '"$2"', '"$3"', $3, $6, $7, test_time} / Sub stats: /{printf "s@ %s@ %s@ %s@ %s@ %s@ %s\n", '"$2"', '"$4"', $3, $6, $7, test_time}' \
	| sed -e 's/,//g' -e 's/@/,/g' \
	>> $5
}
...
do
	run_concurrent_user_test $1 10000 $payload
done
